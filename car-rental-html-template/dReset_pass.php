...
$_SESSION['userpp'] = $id;
if ($_SESSION['userpp'] == '') {
		header("location:dforget_pass.php");
}
else
{
if(isset($_POST['submit'])) {
	...
	$query = "UPDATE driver_register SET dpassword = '$password' WHERE driver_id = '$id_decode' ";
	$result = $conn->query($query);
		if($result){
			$message = "<div class='alert alert-success'>Reset Your Password Successfully..</div>";
			$Home = "<a href='driver_login' class='btn btn-success btn-sm'>Login</a>";
	}else{
